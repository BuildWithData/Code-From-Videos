...
class Analytics:
    ...
    def get_views_from_month(
        ...
        if from_ is not None:
            data = data[data.publish_date >= date.fromisoformat(from_)]

        if to is not None:
            data = data[data.publish_date <= date.fromisoformat(to)]
    ...
    def get_total_minutes_per_month(
        self,
        from_: str = None,
        to: str = None,
        heatmap: bool = True
    ...
    def get_avg_minutes_per_month(
        ...
        if heatmap is True:
            minutes_per_month = self._heatmap(minutes_per_month[list(range(1, 13))])

        return minutes_per_month

    def get_top_videos_by_views(self, top: int) -> DataFrame:
        return self.data.sort_values("views", ascending=False).reset_index(drop=True).head(top)
